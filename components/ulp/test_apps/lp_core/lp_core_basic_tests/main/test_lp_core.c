...
static uint64_t RTC_IRAM_ATTR get_rtc_timer_cycle_count(void)
{
#if SOC_LP_TIMER_SUPPORTED
...
static void RTC_IRAM_ATTR wake_stub_lp_core_test(void)
{
    ...
    if (s_wake_stub_run_count == 1) {
        s_stub_first_rtc_tick = get_rtc_timer_cycle_count();
    }
    ...
    s_stub_last_rtc_tick = get_rtc_timer_cycle_count();
}
